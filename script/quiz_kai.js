...
<h3>${q.question}</h3>
  ...
  if(q.type==="meerkeuze"){
    ...
    html+=`
<div><input type="radio" name="answer" value="true"> Waar</div>
...
<div class="fouten-lijst">
          ${uniekeFouten.map(n => `<div class="fout-badge">Vraag ${n}</div>`).join("")}
</div>
</div>
    `;
  } else {
...
<strong>Perfect!</strong>
        Alles goed beantwoord, geweldig werk!
</div>
    `;
  }
